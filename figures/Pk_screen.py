...
#
mg_rc = [250,500,1000,2500,5000,10000,20000,30000,50000,100000]
...
M = {}
#
for i in range(len(mg_rc)):
    common_settings['rc'] = mg_rc[i]
    #
...
for i in range(len(mg_rc)):
    ...
    h = 0.6766
    n = len(mg_rc)
    var_color = cmap3(np.linspace(0,1,n))[i]
    #
    # get Cls
    #
    ...
    ax_Pk.plot(kvec,pkM[i],
                       color=var_color,#alpha=var_alpha,
                       linestyle='-',
               label=r"$r_c = {}$".format(f.format_data(mg_rc[i])))
...
mg_rc = np.array(mg_rc)
norm = mpl.colors.Normalize(vmin=np.log10(mg_rc.min()), vmax=np.log10(mg_rc.max()))
...
for j, beta in enumerate(beta_arr):
    for i, K0 in enumerate(K0_arr):
        common_settings['beta_kmfl'] = beta
        common_settings['k0_kmfl'] = K0
        #
        # rescale omega_cdm and h
        #
        #
...
plt.savefig('Pk_All2.pdf',bbox_inches='tight')