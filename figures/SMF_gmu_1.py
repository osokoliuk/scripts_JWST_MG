...
for z_smf in z_smf_arr:
    ...
    for j, par1 in enumerate(pars1):
        ...
        data = np.load('./data_folder/SMF_gmu_'+str(model_SFR)+'_z'+str(z_smf)+'_'+str(par1)+'.npz')
    ...
    line3 = ax_Pk.plot([0], [0], label=r'$g_{\mu}=-1$', color=colors[0][-1])   
    line2 = ax_Pk.plot([0], [0], label=r'$g_{\mu}=0$', color=colors[1][-1])
    line1 = ax_Pk.plot([0], [0], label=r'$g_{\mu}=1$', color=colors[2][-1])
