...
data = np.loadtxt('/home/oleksii/codes/scripts_JWST_MG/observational_data/SFRD/sfrd_new_all.dat', usecols=(1,2,3,4,))
x = (data[:,0] + data[:,1])/2
y = data[:,2]
yerr = data[:,3]
marker = '.'
color = 'tab:gray'
plt.errorbar(x,y,yerr=yerr, capsize=2,ecolor=color,color='w',marker=marker,markersize=6,markeredgewidth=1.3, elinewidth=1,ls='None',markeredgecolor=color, zorder= 3)
...
for i in range(len(SFRD_obs)):
    plt.plot(z_int, SFRD_obs[i], c = colors[i], lw = 1)
plt.yscale('log')
